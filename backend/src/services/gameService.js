...
async function getOrCreateRTPProfile(playerId) {
  ...
  if (!profile) {
    profile = await prisma.rTPProfile.create({
      data: { playerId },
    });
  }
  return profile;
...
async function processSpin(playerId, betAmount, sessionId, io) {
  ...
  let newLosingStreak = outcome.isWin ? 0 : rtpProfile.losingStreak + 1;
  let newWinningStreak = outcome.isWin ? rtpProfile.winningStreak + 1 : 0;
  let newPityCounter = outcome.isWin ? 0 : rtpProfile.pityCounter + 1;
  ...
  const [round] = await prisma.$transaction([
    ...
    prisma.player.update({
      ...
      data: {
        ...
        highestSingleWin: outcome.isWin && payout > player.highestSingleWin
          ? payout
          : undefined,
      },
    }),
    prisma.gameSession.update({
      ...
      data: {
        ...
        longestWinStreak: newWinningStreak > session.longestWinStreak
          ? newWinningStreak
          : undefined,
        longestLoseStreak: newLosingStreak > session.longestLoseStreak
          ? newLosingStreak
          : undefined,
      },
    }),
